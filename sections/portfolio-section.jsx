...
function TickerStat({ value, label }) {
  ...
  return (
    <motion.div className="text-center" onViewportEnter={() => setStarted(true)}>
      <motion.span
...
const SKILLS = ["Graphic Design", "3D Work", "Motion Graphics", "Logo Design", "Video Editing", "Web Design"];
  ...
  if (cell.type === "text") {
    return (
      <motion.div
        ...
        viewport={{ once: true }}
        transition={{ delay: index * 0.08, type: "spring", stiffness: 280, damping: 70 }}
      ...
      >
        ...
        <motion.div
          ...
          style={{
            ...
            background: "conic-gradient(from 0deg, transparent 60%, #89D1D1 75%, #908FDB 85%, transparent 100%)",
        ...
        <div style={{
          ...
          backdropFilter: "blur(12px)", zIndex: 0, pointerEvents: "none",
        }} />
        {/* Teal orb */}
        <motion.div
        ...
        >
          <TickerStat value={30} label="Projects" />
          <div style={{ width: 1, background: "rgba(255,255,255,0.1)" }} />
          <TickerStat value={6} label="Categories" />
        </motion.div>
        {/* CTA */}
        <motion.a
          href="/work"
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: "12px", fontWeight: 700, color: "white", lineHeight: 1.3 }}>
          {cell.title}
        </p>
        <p style={{ margin: "2px 0 0", fontSize: "10px", color: "rgba(255,255,255,0.65)", lineHeight: 1.3 }}>
          {cell.category}
        </p>
      </div>
    </motion.div>
...
function MobileImageCard({ cell, index }) {
  ...
  return (
    <motion.div
      ...
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ delay: index * 0.07, type: "spring", stiffness: 260, damping: 60 }}
      onClick={() => setTapped(t => !t)}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src={cell.src} alt={cell.alt}
        style={{
          width: "100%", height: "100%", objectFit: "cover", display: "block",
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "10px 14px" }}>
        {cell.badge && (
          <span style={{
            ...
            textTransform: "uppercase", color: cell.badge === "new" ? "#89D1D1" : "#908FDB",
            border: `1px solid ${cell.badge === "new" ? "#89D1D180" : "#908FDB80"}`,
            borderRadius: 4, padding: "2px 6px", marginBottom: 4, width: "fit-content",
          }}>{cell.badge}</span>
        )}
        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "white", lineHeight: 1.3 }}>{cell.title}</p>
        <p style={{ margin: "2px 0 0", fontSize: 10, color: "rgba(255,255,255,0.6)", lineHeight: 1.3 }}>{cell.category}</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        opacity: hovered ? 1 : 0.85, transition: "transform 0.3s ease, opacity 0.3s ease",
      }}>
        {cell.badge && (
          <span style={{
            display: "block", fontSize: "9px", fontWeight: 700, letterSpacing: "0.1em",
...
function HeaderCard({ className = "", children }) {
  return (
    ...
    >
      ...
      />
      <div style={{ position: "absolute", inset: 1, borderRadius: 16, backdropFilter: "blur(12px)", zIndex: 0, pointerEvents: "none" }} />
      ...
      >
        ...
        <div className="sm:hidden">
          <HeaderCard className="p-5 mb-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-[#89D1D1] font-semibold tracking-widest uppercase mb-1">My Portfolio</p>
                <SkillTicker />
              </div>
          ...
          <div className="flex flex-col gap-3">
            {imageCells.map((cell, i) => <MobileImageCard key={i} cell={cell} index={i} />)}
          </div>
          <div className="flex flex-col gap-3 mt-3">
            <a href="https://www.instagram.com/sonn.chidiya/" target="_blank" rel="noopener noreferrer" className="btn glass flex items-center justify-center gap-2">
              @sonn.chidiya — Instagram <ExternalLinkIcon className="size-4 shrink-0" />
            </a>
            <a href="https://www.youtube.com/@TheGhostByteGaming?sub_confirmation=1" target="_blank" rel="noopener noreferrer" className="btn glass flex items-center justify-center gap-2">
              Ghost Byte Channel <ExternalLinkIcon className="size-4 shrink-0" />
            </a>
          </div>
        </div>
        ...
        <div className="hidden sm:block lg:hidden">
          ...
          <div className="grid grid-cols-2 gap-3">
            {imageCells.map((cell, i) => <TabletImageCard key={i} cell={cell} index={i} />)}
          </div>
          <div className="flex gap-3 mt-3">
            <a href="https://www.instagram.com/sonn.chidiya/" target="_blank" rel="noopener noreferrer" className="btn glass flex items-center justify-center gap-2 flex-1">
              @sonn.chidiya — Instagram <ExternalLinkIcon className="size-4 shrink-0" />
            </a>
            <a href="https://www.youtube.com/@TheGhostByteGaming?sub_confirmation=1" target="_blank" rel="noopener noreferrer" className="btn glass flex items-center justify-center gap-2 flex-1">
              Ghost Byte Channel <ExternalLinkIcon className="size-4 shrink-0" />
            </a>
          </div>
        </div>
        ...
        <div className="hidden lg:block">
          <div className="grid grid-cols-3 grid-rows-3 gap-3 h-[520px]">
            {cells.map((cell, i) => <BentoCell key={i} cell={cell} index={i} />)}
          </div>
